...
3. **All `@effectstream/*` packages share a single coordinated version.** Never mix `0.100.13` and `0.100.5` across `@effectstream/*` deps. To find the version to pin, **query npm first** — `npm view @effectstream/orchestrator version` (any `@effectstream/*` package works; they're always co-versioned). If npm returns no result (the engine release isn't published yet), **fall back to the `"version"` field of the engine monorepo's root `package.json`** at `/Users/edwardalvarado/pe-bun/package.json`. Use the resolved version for every `@effectstream/*` dep across the template.
...
| 9 | (optional) `config.mainnet.ts`, `main.mainnet.ts`, `batcher.mainnet.ts`, `"start:mainnet"` script. | `references/multi-env.md` |
| 10 | `Dockerfile` + `.dockerignore`. | `references/docker.md` |
| 11 | `README.md` at project root following the canonical structure. | `references/readme.md` |
| 12 | Verify: `bun install && bun run dev` boots the full stack, `bun run test` passes, `docker build` succeeds, `docker run <image> bun run test` passes. | — |
...
6. **`docker build`** — catches missing system deps (`procps`, `lsof`, `iproute2`, `xz-utils` for Midnight, Foundry for EVM) and the workspace-symlink workaround. Bun on Linux does NOT resolve workspace siblings the way it does on Mac, so the Dockerfile MUST create `node_modules/<scope>/<pkg>` symlinks inline after `bun install` — see `references/docker.md`.
7. **`docker run <image> bun run test`** — catches `cwd` vs `resolveFrom` issues in the orchestrator config.

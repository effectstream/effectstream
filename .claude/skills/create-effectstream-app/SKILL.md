...
8. **`BatcherConfig.namespace` and the frontend's `EffectstreamConfig` `securityNamespace` (first constructor arg) must match exactly.** A mismatch produces `401 Invalid signature` from the batcher's `/send-input` endpoint — the signed message contains `securityNamespace`, the batcher validates against `namespace`. (Not the runtime's `start({ appName })` — a different field.)
...
- **`401 Invalid signature` from batcher `/send-input`** → `BatcherConfig.namespace` ≠ frontend `EffectstreamConfig` `securityNamespace`. Both must equal the same string (often `""` or the template name). → `references/batcher.md`

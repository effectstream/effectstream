...
/**
 * Wait until every dust coin that could be SELECTED to pay a fee can actually
 * cover one.
 *
 * The wallet SDK picks the smallest coin with value > 0. A UTXO that just
 * arrived has a near-empty dust coin, so the selector prefers it over a mature
 * one and balancing fails with "could not balance dust" even though the wallet
 * holds plenty of dust in aggregate. Coins at exactly 0 are skipped by the
 * selector and are therefore harmless.
 */
export async function waitForSelectableDust(
  ctx: WalletCtx,
  minValuePerCoin: bigint,
  timeoutMs = SYNC_TIMEOUT_MS,
): Promise<void> {
  ...
  for (;;) {
    const info = await getDustCoins(ctx, minValuePerCoin);
    const positives = info.values.filter((v) => v > 0n);
    const smallest = positives.length > 0
      ? positives.reduce((a, b) => (a < b ? a : b))
      : 0n;
    if (positives.length > 0 && smallest >= minValuePerCoin) return;
    if (Date.now() - start > timeoutMs) {
      throw new Error(
        `timeout waiting for selectable dust (smallest positive coin ${smallest} < ${minValuePerCoin}; ` +
          `values=${info.values.map(String).join(",")})`,
      );
    }
    await new Promise((r) => setTimeout(r, 2_000));
  }
...
/**
 * Build an UNBALANCED swap offer via `initSwap`: declare the tokens you want
 * to receive (`want`) and the outputs you provide (`give`). The resulting
 * transaction carries non-zero shielded deltas — that asymmetry is what makes
 * it a *swap offer* rather than a transfer, and what product-c's custom filter
 * inspects.
 *
 * Returned unproven (the batcher proves + balances it).
 */
/**
 * A swap OFFER: half a trade, deliberately unbalanced.
 *
 * `spend` is what the maker actually PAYS — the wallet sources these coins, so
 * it must hold them. `create` is what the offer CREATES, which may be a token
 * the maker does not own: that is the side a counterparty (or solver) has to
 * supply for the offer to settle.
 *
 * Net deltas are inputs − outputs, so `spend` shows up as +X and `create` as
 * −Y. Spending one token and creating another in equal amount is therefore the
 * `+X tokenA / −X tokenB` matched-delta shape.
 */
export async function buildSwapOffer(
  ctx: WalletCtx,
  spend: Record<string, bigint>,
  create: Array<{ token: string; amount: bigint; receiverAddress: unknown }>,
): Promise<UnprovenTransaction> {
  const recipe = await (ctx.wallet as unknown as {
    initSwap: (
      desiredInputs: { shielded: Record<string, bigint> },
      desiredOutputs: unknown[],
      keys: unknown,
      opts: { ttl: Date; payFees?: boolean },
    ) => Promise<{ transaction: UnprovenTransaction }>;
  }).initSwap(
    { shielded: spend },
    [{
      type: "shielded",
      outputs: create.map((c) => ({
        type: c.token,
        amount: c.amount,
  ...
  return await ctx.wallet.signUnprovenTransaction(
    recipe.transaction,
    (payload: Uint8Array) => ctx.unshieldedKeystore.signData(payload),
  );
}

/** Shielded token id (native) — the only token type on a bare dev chain. */
export function shieldedTokenId(): string {
  return shieldedToken().raw;
}

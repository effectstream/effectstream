...
async function test() {
  ...
  // Track exceptions thrown before any failed assertion — see fix(templates/minimal)
  // for the full story; without this flag the runner silently exits 0 whenever
  // Phase A passes >=1 test and Phase B/C then throws before incrementing failCount.
  ...
  try {
    ...
    const { sendMessageTest } = await import("./stm/send-message.test.ts");
    await sendMessageTest(db);

    const { gateTest } = await import("./stm/gate.test.ts");
    await gateTest();

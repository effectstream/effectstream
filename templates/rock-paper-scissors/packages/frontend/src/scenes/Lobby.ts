import mw, { WalletMode } from '../../paimaMiddleware.js';

// Type for lobby data returned from API
interface QueryLobby {
  lobby_id: string;
...
export class Lobby extends Phaser.Scene {
  ...
  async updateLobbyUI() {
    ...
    mw.getUserLobbies(wallet.result.walletAddress, 1, 100).then(myLobbies => {
    ...
    mw.getOpenLobbies(1, 10).then(lobbies => {
      if (lobbies.success) {

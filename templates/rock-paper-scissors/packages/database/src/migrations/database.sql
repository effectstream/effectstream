...
-- Note: starting_block_height / execution_block_height are plain integers (no
-- FK to block_heights). The engine does not guarantee the current block's
-- block_heights row exists at the moment a state transition runs, so an FK here
-- would abort the transition's transaction.
CREATE TABLE rounds(

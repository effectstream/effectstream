...
async function test() {
  ...
  try {
    ...
    const {
      ...
      submittedMovesTest,
      closedLobbyTest,
      zombieScheduledDataTest,
      userScheduledDataTest,
    ...
    await closedLobbyTest(db);
    await zombieScheduledDataTest(db);
    await userScheduledDataTest(db);
    ...
    // Smoke e2e: drives the local-JS wallet (EvmViem) through connect → start
    // scene visible in headless Chromium. The RPS frontend was Phaser-based; per
    // the test plan, Phase C is a minimal "wallet connect + start scene visible"
    // check rather than a pixel-perfect canvas drive.

...
async function waitForBatcher(timeoutMs = 120_000): Promise<void> {
  console.log("Waiting for batcher...");
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    try {
  ...
  throw new Error("Batcher did not become reachable");
}
...
async function test() {
  ...
  try {
    ...
    console.log("Batcher is reachable.\n");
    ...
    const { changeNameTest, gainExperienceDirectTest } =
      await import("./stm/actions.test.ts");
    await changeNameTest(db);
    await gainExperienceDirectTest(db);

    // The web2.5 path: a gasless, batcher-submitted gainExperience lands on-chain
    // and credits the signing user's DB row.
    const { batchedGainExperienceTest } = await import("./stm/batched.test.ts");
    await batchedGainExperienceTest(db);

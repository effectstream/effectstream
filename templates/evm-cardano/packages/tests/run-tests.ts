...
async function test() {
  ...
  try {
    ...
    const { evmNftTest } = await import("./stm/evm-nft.test.ts");
    await evmNftTest(db);

    const { cardanoTransferTest } = await import("./stm/cardano-transfer.test.ts");
    await cardanoTransferTest(db);
    ...
    const installProc = Bun.spawn(["bunx", "playwright", "install", "chromium"], {
      cwd: frontendDir,
      stdout: "inherit",
      stderr: "inherit",
    });
    ...
    await assert("Playwright E2E tests pass", async () => playwrightExit === 0);

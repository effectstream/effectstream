# Dice Game — Effectstream Template
...
|---------|------|---------|
| `packages/node/` | `@dice/node` | Sync node, grammar, state machine, dice game helpers, API |
| `packages/database/` | `@dice/database` | SQL migrations + pgtyped queries |
| `packages/contracts-evm/` | `@dice/contracts-evm` | EffectstreamL2 + AnnotatedMintNft (ERC721) contracts |
| `packages/frontend/` | `@dice/frontend` | Vanilla-JS dice UI + Fastify static server, `@effectstream/wallets` |
| `packages/tests/` | `@dice/tests` | E2E test suite (Phase A infra, B STM/DB/API, C frontend) |

...
import {
  verifyPrefundedWallets,
  type GeneratedWalletState,
} from "./verify-prefunded-wallets.ts";
...
async function loadWallets(): Promise<GeneratedWalletState[]> {
  const currentDir = process.cwd();
  const wallets: GeneratedWalletState[] = [];
...
export async function mintWallets(): Promise<void> {
  ...
  // NIGHT and DUST are part of the pinned custom genesis. Verify the exact
  // profile read-only and fail closed; never hide a mismatch by falling back
  // to the old serial faucet/registration path.
  await verifyPrefundedWallets(wallets);

  // Pre-mint M20 inventory to each filler via the new mint_unshielded
  ...
  console.log(
    "Filler genesis NIGHT/DUST verification + M20 pre-mint complete (no live NIGHT provisioning).",
  );
}

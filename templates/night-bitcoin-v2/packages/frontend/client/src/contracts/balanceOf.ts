...
const NETWORK_ID = import.meta.env.VITE_MIDNIGHT_NETWORK_ID || "undeployed";

const getContractAddress = async (): Promise<string> => {
  const r = await fetch(`contract_address/unshielded-erc20.${NETWORK_ID}.json`);
...
export async function balanceOf(address: string): Promise<bigint> {
  ...
  const publicStates = await getContractState(
    config.indexer,
    config.indexerWS,
    contractAddress,
    "balanceOf.ts",

...
/**
 *  Continuously generate regtest blocks against a local Bitcoin Core node.
 *
 *  Usage:
 *  bun run generate-blocks.ts [--block-interval 5000]
 */
...
function generateMockAddress(): string {
  ...
  const mockPayment = bitcoin.payments.p2wpkh({
    pubkey: Buffer.from(mockKeyPair.publicKey),
    network: NETWORK,
  });
...
async function main() {
  ...
  // Initialization Phase: generate enough blocks to mature coinbase rewards.
  ...
  const initialBlocks = await bitcoinRpcCall(
    "generatetoaddress",
    [101, defaultWalletAddress],
    walletName
  );

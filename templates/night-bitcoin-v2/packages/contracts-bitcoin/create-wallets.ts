 ...
 *  Usage:
 *  bun run create-wallets.ts 1.5 10 101 0xf8d62248c2abdacd7550e7a6cd5a9de
 *
 *  Arguments:
...
if (import.meta.main) {
  ...
  // Resolve generated/ relative to this script's location, not cwd, so the
  // wallet paths match what start.dev.ts computes (packages/contracts-bitcoin/generated).
  const scriptDir = import.meta.dirname!;
  const { mkdir } = await import("node:fs/promises");
  const { writeFileSync } = await import("node:fs");
  await mkdir(path.join(scriptDir, "generated"), { recursive: true });
  for (let i = 0; i < wallets.length; i++) {
    const wallet = wallets[i];
    const outputPath = path.join(scriptDir, "generated", `wallet-${i}.json`);

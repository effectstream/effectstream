...
The template ships a checksum-verified custom `undeployed` genesis built from
...
each with five NIGHT UTXOs already registered for DUST. Every launch verifies the
bundled snapshot, then starts it through the normal `@effectstream/npm-midnight-node`
binary wrapper. This skips three serial NIGHT transfers, the fixed three-minute aging
wait and three serial DUST-registration transactions. Only the three
application-specific M20 inventory mints remain.

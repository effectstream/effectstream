...
import {
  ...
  witnesses,
} from "@midnight-batcher/midnight-contract";

import { NETWORK, SEEDS } from "./env.ts";
import { sendTx, waitForBatcher } from "./batcher-client.ts";
import { deriveSeedForRole, toHex } from "./wallet.ts";
...
const CONTRACTS_DIR = path.join(import.meta.dirname!, "../contracts-midnight");
...
function buildProviders() {
  ...
  const keys = ZswapSecretKeys.fromSeed(deriveSeedForRole(SEEDS.callCaller, Roles.Zswap));
...
async function main() {
  ...
  const { contractAddress } = JSON.parse(readFileSync(ADDRESS_FILE, "utf8"));
  console.log(`[calls] contract: ${contractAddress}`);
  ...
  console.log("[calls] finding deployed contract...");
  ...
  if (VERIFY) console.log(`[calls] counter before: ${before}`);

  console.log(`[calls] submitting ${COUNT} increment calls (concurrency=${CONCURRENCY})...`);
  let accepted = 0;
  let rejected = 0;
  let buildFailed = 0;
  ...
  const workers = Array.from({ length: Math.min(CONCURRENCY, COUNT) }, async () => {
    for (;;) {
      ...
      try {
        ...
        buildFailed += 1;
        console.error(`[calls] #${i}: pipeline completed without capture (unexpected)`);
        continue;
      } catch (e) {
        hex = capturedTxs.shift();
        if (!hex) {
          buildFailed += 1;
          console.error(`[calls] #${i}: build failed: ${e instanceof Error ? e.message : e}`);
          continue;
        }
      }
      const buildMs = Math.round(performance.now() - buildStart);
      const result = await sendTx(hex, { txStage: "unbound", address: "call-caller" });
      if (result.ok) {
        accepted += 1;
        console.log(`[calls] #${i}: accepted (build+prove=${buildMs}ms, ${accepted}/${COUNT})`);
      } else {
        rejected += 1;
        console.error(`[calls] #${i}: REJECTED status=${result.status} body=${JSON.stringify(result.body)}`);
      }
    }
  });
  await Promise.all(workers);
  const wallMs = Math.round(performance.now() - t0);
  console.log(`[calls] submit done: accepted=${accepted} rejected=${rejected} buildFailed=${buildFailed} in ${wallMs}ms`);

  let delivered: number | null = null;
  if (VERIFY) {
    const want = before + BigInt(accepted);
    console.log(`[calls] verifying: waiting for counter ${before} → ${want}...`);
    ...
    for (;;) {
      ...
      if (Date.now() - start > VERIFY_TIMEOUT_MS) {
        console.error(`[calls] VERIFY TIMEOUT: delivered ${delivered}/${accepted}`);
        break;
      }
      console.log(`[calls] delivered so far: ${delivered}/${accepted}`);
      await new Promise((r) => setTimeout(r, 5_000));
    }
    console.log(`[calls] delivered: ${delivered}/${accepted}`);
  }

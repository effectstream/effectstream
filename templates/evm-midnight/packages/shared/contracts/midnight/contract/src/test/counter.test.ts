...
describe("Counter smart contract", () => {
  ...
  it("increments the counter correctly", () => {
    ...
    expect(nextLedgerState.property_name).toEqual(Uint8Array.from("test A".padEnd(128, " ").split("").map((c) => c.charCodeAt(0))));
    expect(nextLedgerState.value).toEqual(Uint8Array.from("test B".padEnd(128, " ").split("").map((c) => c.charCodeAt(0))));

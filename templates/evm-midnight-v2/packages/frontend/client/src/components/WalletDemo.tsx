...
import { erc721dev } from "@evm-midnight/contracts-evm";
...
const convertApiDataToTokens = (
  ...
  const tokenMap: Record<string, ERC721Token> = {};
  for (const data of apiData) {
    if (!tokenMap[data.token_id]) {
      tokenMap[data.token_id] = {
        id: data.token_id,
        owner: data.owner,
        properties: {},
        isValid: !!data.owner,
    ...
    if (data.property_name) {
      tokenMap[data.token_id].properties[data.property_name] = data.value;
    }
  }
  return Object.values(tokenMap);
};

function areTokensEqual(a: ERC721Token[], b: ERC721Token[]): boolean {
  ...
  for (const [id, tokenA] of aMap.entries()) {
    const tokenB = bMap.get(id);
    if (!tokenB) return false;
    if (tokenA.owner !== tokenB.owner) return false;
    if (tokenA.isValid !== tokenB.isValid) return false;

    const propsA = tokenA.properties;
    const propsB = tokenB.properties;
...
export function WalletDemo() {
  ...
  const loadTokens = async () => {
    ...
    try {
      ...
      setTokens((prevTokens) => {
        if (!areTokensEqual(prevTokens, convertedTokens)) {
  ...
  useEffect(() => {
    // We only want to load tokens when no token is selected
    if (!selectedToken) {
      loadTokens();
    }

    const interval = setInterval(() => {
      // We pause refreshing when a token is selected to avoid losing focus on input fields
      if (!selectedToken) {
        loadTokens();
      }
    }, 5000);
  ...
  const handleCardSelect = (tokenId: string) => {
    setSelectedToken((prevSelectedToken) =>
      prevSelectedToken === tokenId ? null : tokenId
    );
    setNewPropName("");
    setNewPropValue("");
  };
  ...
  const addProperty = async (
    ...
    const propertyKey = `${tokenId}-${propertyName}`;
    ...
    try {
      ...
      console.log("📝 [PROPERTY] Contract:", erc721Address);
      ...
      showNotification(
        "info",
        "Adding Property",
        `Adding "${propertyName}" property to NFT on Midnight network...`,
      );
      ...
      showNotification(
        "success",
        "Property Added",
        `Property "${propertyName}: ${propertyValue}" added to NFT successfully! Transaction: ${result.txId}. Data will refresh from API.`,
      );

      const pollForProperty = async (attempts = 0, maxAttempts = 60) => {
        try {
          const apiData = await fetchNFTData();
          const convertedTokens = convertApiDataToTokens(apiData);
          const updatedToken = convertedTokens.find(
            (t) => t.id === tokenId,
          );
          if (updatedToken && propertyName in updatedToken.properties) {
            setTokens(convertedTokens);
            return;
          }
        } catch {}
        if (attempts < maxAttempts) {
          setTimeout(() => pollForProperty(attempts + 1, maxAttempts), 2000);
        }
      };
      pollForProperty();
    } catch (error) {
      ...
      showNotification(
        ...
        `Failed to add property "${propertyName}: ${propertyValue}" to NFT: ${
  ...
  return (
    <div className="wallet-demo-section">
      ...
      <div className="wallet-demo-header">
        <h2 className="wallet-demo-title">
          Multi-Chain Wallet Demo: EVM ↔ Midnight Integration
        </h2>
        <p className="wallet-demo-subtitle">
          EVM Wallet creates & transfers ERC721 Tokens. Midnight Wallet adds metadata across blockchains.
        </p>
        <div className="wallet-demo-status-bar">
          <div className="status-chip evm-chip">
            {walletConnected && wallet
              ? <>
                  <span className="status-dot connected" />
                  <span className="status-address">{walletAddress?.slice(0, 6)}...{walletAddress?.slice(-4)}</span>
                </>
              : <button type="button" onClick={openModal} className="status-connect-btn evm-connect-btn">
                  Connect EVM
                </button>
            }
          </div>
          <div className="status-chip midnight-chip">
            {midnightWallet?.connected
              ? <>
                  <span className="status-dot connected" />
                  <span className="status-address">{midnightWallet.address?.slice(0, 6)}...{midnightWallet.address?.slice(-4)}</span>
                </>
              : <button
                  type="button"
                  onClick={connectMidnightWalletHandler}
                  className="status-connect-btn midnight-connect-btn"
        ...
        <div className="wallet-demo-content">
          ...
          <div className="tokens-cards-container">
            <h3 className="cards-title">
              Merged EVM ERC721 + Midnight Properties
              {isLoadingTokens && (
            ...
            {walletConnected && (
              <div className="token-creation-row">
                ...
                <button
                  type="button"
                  onClick={() => setCreateTokenId(Date.now().toString())}
                ...
                >
                  {isCreatingToken
                    ? <><span className="loader"></span>Minting...</>
                    : "MINT"
                  }
                </button>
                {createTokenId.trim() &&
                                    ...
                                    .map((
                                      ...
                                      <button
                                        ...
                                        onClick={(e) => {
                                          ...
                                          setNewPropName(suggestion.key);
                                          setNewPropValue(suggestion.value);
                                        }}
                        ...
                        {selectedToken === token.id && (
                          ...
                          >
                            ...
                            >
                              ...
                              <div style={{ display: "flex", gap: "10px" }}>
                                <input
                                  className="token-name-input evm-input"
                                  style={{ flex: 1 }}
                                  value={(token.properties.Name as string) ||
                                    ""}
                                  onChange={(e) => {
                                    ...
                                    if (tokenIndex !== -1) {
                                      newTokens[tokenIndex] = {
                                        ...newTokens[tokenIndex],
                                        properties: {
                                          ...newTokens[tokenIndex].properties,
                                          Name: e.target.value,
                                        },
                                      };
                                      setTokens(newTokens);
                                ...
                                <button
                                  ...
                                  disabled={isIncrementingCounter || !midnightWallet?.connected}
                                  onClick={async () => {
                                    await addProperty(
                                      token.id,
                                      "Name",
                                      token.properties.Name as string,
                                    );
                                  }}
                                >
                                  {!midnightWallet?.connected ? "Connect Midnight" : isIncrementingCounter ? "Saving..." : "Save"}
                                </button>
                              </div>
                            </div>
                            ...
                            >
                                ...
                                onClick={async () => {
                                  if (!newPropName || !newPropValue) {
                                    showNotification(
                                      "error",
                                      "Missing input",
                                      "Please provide both property name and value",
                                    );
                                    return;
                                  }
                                  await addProperty(
                              ...
                              >
                                {!midnightWallet?.connected ? "Connect Midnight" : isIncrementingCounter ? "Adding..." : "Add"}
                              </button>
                            </div>
                          </div>
                        )}
      ...
      <style>
        {`
          ...
          .wallet-demo-title {
            font-size: 1.8rem;
            font-weight: 700;
            background: linear-gradient(45deg, #93c5fd, #e0e7ff);
          ...
          .wallet-demo-subtitle {
            color: #94a3b8;
            font-size: 0.95rem;
            margin: 0;
          }

          .wallet-demo-status-bar {
            display: flex;
            gap: 12px;
            margin-top: 8px;
          }
          ...
          .token-creation-row .token-id-label {
            font-weight: 600;
            color: #0f172a;
            font-size: 0.9rem;
            white-space: nowrap;
          ...
          .token-creation-row .token-name-input {
            flex: 1;
            min-width: 100px;
            max-width: 180px;
          }

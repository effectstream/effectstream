...
async function test() {
  ...
  try {
    ...
    console.log("Cardano transactions submitted (Lock→Unlock→Claim complete).");
    ...
    // ── Phase B: State Machine + API ────────────────────────────────────
    console.log("\n--- Phase B: State Machine + API Tests ---\n");
    db = getDBConnection();

    const { projectedNftTest } = await import("./stm/projected-nft.test.ts");
    ...
    const { playwrightE2eTest } = await import("./frontend/playwright-e2e.test.ts");
    await playwrightE2eTest();

# Generic - Effectstream Template

A minimal single-action Effectstream app, ported from the paima-engine-v1
`generic` game template. It demonstrates the smallest meaningful game loop:
...
│
└── unity-client/                # the ORIGINAL Unity game client (see below)
```
...
## The Unity client (`unity-client/`)

The original paima-engine-v1 `generic` template shipped a **Unity** game client,
not a web app. That project is preserved verbatim under `unity-client/`
(`Assets/`, `Packages/`, `ProjectSettings/`). It is the rich game client and is
built / run **separately in the Unity Editor** — this template's `bun run dev`
does **not** build or serve it.

In v1 the Unity build talked to the node through a bundled JS middleware
(`document.Paima` global) injected into the WebGL template. In the Effectstream
v2 model that middleware is gone: the Unity client should talk to the node via
the same surface the web frontend uses — `@effectstream/wallets`
(`sendTransaction` for the `gainedExperience` action) for writes, and the
read-only HTTP API (`GET /user_state?wallet=…`) for reads. Wiring that into the
Unity WebGL `.jslib` bridge is left as an exercise; see `unity-client/README.md`
for the original Unity/WebGL build + deploy notes.

...
type MultiChainMultiTokenCircuits = ImpureCircuitId<
  MultiChainMultiToken.Contract
>;
...
const getMultiChainMultiTokenLedgerState = async (
  ...
  try {
    ...
    console.error(
      "❌ Error getting multi chain multi token ledger state:",
      error,
    );
    throw error;
  }
...
const balanceOf = async (
  ...
  const accountBalance = await multiChainMultiTokenContract.callTx.balanceOf(
    either,
  );
...
const mint = async (
  ...
  const finalizedTxData = await multiChainMultiTokenContract.callTx.mint(
    either,
    value,
  );
  console.log(
...
async function joinAndMint(account: string, amount: bigint): Promise<void> {
  ...
  try {
    ...
    const multiChainMultiTokenContract = await joinContract(
      providers,
      contractAddress,
    );
    ...
    const incrementResult = await mint(
      multiChainMultiTokenContract,
      account,
      amount,
    );
    ...
    const accountBalance = await balanceOf(
      multiChainMultiTokenContract,
      account,
    );

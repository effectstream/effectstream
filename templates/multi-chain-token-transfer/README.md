...
> A unified ERC1155 balance across a local EVM chain and Midnight, moved by a custom sync primitive and a batcher that writes to both chains.
...
![The dApp UI, showing EVM and Midnight balances side by side](./docs/ui.png)
...
[`packages/shared/custom-primitive-mct-erc1155/erc1155-primitive.ts`](./packages/shared/custom-primitive-mct-erc1155/erc1155-primitive.ts)
...
[`packages/client/node/src/main.ts`](./packages/client/node/src/main.ts):
...
[`packages/shared/data-types/src/localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts),
...
[`erc1155-grammar.ts`](./packages/shared/custom-primitive-mct-erc1155/erc1155-grammar.ts)
...
| --- | --- | --- |
| Custom primitive (`PaimaPrimitive` subclass) | [`packages/shared/custom-primitive-mct-erc1155/erc1155-primitive.ts`](./packages/shared/custom-primitive-mct-erc1155/erc1155-primitive.ts) | Decoding the contract's own `TransferToMidnight` event, which no built-in primitive models |
| `userDefinedPrimitives` registration | [`packages/client/node/src/main.ts`](./packages/client/node/src/main.ts) | Making the `EVM:MCT_ERC1155` type usable from configuration |
| Built-in `PrimitiveTypeEVMERC1155` | [`packages/shared/data-types/src/localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts) | Tracking ordinary ERC1155 mints, burns and transfers on the EVM chain |
| Built-in `PrimitiveTypeMidnightGeneric` | [`packages/shared/data-types/src/localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts) | Reading the Compact contract's public ledger fields each block |
| NTP main sync protocol | [`packages/shared/data-types/src/localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts) | A wall-clock main chain that both parallel chains are ordered against |
| Concise grammar (`@paimaexample/concise`) | [`packages/shared/data-types/src/grammar.ts`](./packages/shared/data-types/src/grammar.ts) | Typing the three input streams reaching the state machine |
| State machine (`PaimaSTM` from `@paimaexample/sm`) | [`packages/client/node/src/state-machine.ts`](./packages/client/node/src/state-machine.ts) | Reacting to each chain's events and issuing the counter-mint |
| Batcher (`@paimaexample/batcher`) with two adapters | [`packages/client/batcher/`](./packages/client/batcher/) | Holding the minting authority for both chains behind one HTTP endpoint |
| Custom `BlockchainAdapter` | [`packages/client/batcher/erc1155-adapter.ts`](./packages/client/batcher/erc1155-adapter.ts) | Calling named contract functions instead of submitting L2 inputs |
| Built-in `MidnightAdapter` | [`packages/client/batcher/config.ts`](./packages/client/batcher/config.ts) | Proving and submitting the `mint` circuit on Midnight |
| Custom API routes (`StartConfigApiRouter`) | [`packages/client/node/src/api.ts`](./packages/client/node/src/api.ts) | `/api/erc1155` for the unified balance table, `/api/faucet` for local tDUST |
| Migrations and pgtyped queries | [`packages/client/database/`](./packages/client/database/) | The `evm_midnight` balance table and its typed accessors |
| Process orchestrator (`@paimaexample/orchestrator`) | [`packages/client/node/scripts/start.ts`](./packages/client/node/scripts/start.ts) | Bringing up both chains, the batcher, the explorer and the frontend |
| Wallet login (`@paimaexample/wallets`) | [`packages/frontend/client/src/paima.ts`](./packages/frontend/client/src/paima.ts) | `WalletMode.EvmInjected` and `WalletMode.Midnight` in one UI |
...
[`../check.sh`](../check.sh) script verifies all of them.
...
The template also ships a [`Dockerfile`](./Dockerfile) that installs Foundry and the Compact
...
[`ERC1155.sol`](./packages/shared/contracts/evm/src/contracts/ERC1155.sol) is a standard
...
[`multichain_multitoken.compact`](./packages/shared/contracts/midnight/contract-eip-1155/src/multichain_multitoken.compact)
...
[`localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts) builds the whole
...
[`state-machine.ts`](./packages/client/node/src/state-machine.ts) registers one transition
...
[`main.ts`](./packages/client/batcher/main.ts), not in the config object:
...
[`erc1155-adapter.ts`](./packages/client/batcher/erc1155-adapter.ts) is a custom
...
[`calls.ts`](./packages/client/batcher/calls.ts) is the state machine's side of that
...
([`database.sql`](./packages/client/database/src/migrations/database.sql)):
...
[`sm_example.sql`](./packages/client/database/src/sql/sm_example.sql) become typed
...
![The generated OpenAPI documentation for the node's routes](./docs/api.png)

[`api.ts`](./packages/client/node/src/api.ts) registers two routes on the node's Fastify
...
([`faucet.ts`](./packages/shared/contracts/midnight/faucet.ts)) sends 10 tDUST from the
...
[`App.tsx`](./packages/frontend/client/src/App.tsx) reads balances from three places at
once, which is the point of the UI: the EVM contract directly (`evm_balanceOf` via Viem),
the Midnight indexer directly ([`balanceOf.ts`](./packages/frontend/client/src/balanceOf.ts)),
...
[`paima.ts`](./packages/frontend/client/src/paima.ts) — `WalletMode.EvmInjected` and
`WalletMode.Midnight` — after which the Midnight side hands off to
`eip-1155-interact.ts` to build providers and call circuits.

### Startup

![The orchestrator TUI running the full stack](./docs/terminal.png)

`bun run dev` runs [`scripts/start.ts`](./packages/client/node/scripts/start.ts). This
...
Finally [`main.ts`](./packages/client/node/src/main.ts) starts the engine inside the static
...
| --- | --- | --- |
| `BATCHER_PORT` | [`batcher/config.ts`](./packages/client/batcher/config.ts), [`batcher/calls.ts`](./packages/client/batcher/calls.ts) | `3334` |
| `BATCHER_PRIVATE_KEY` | [`batcher/config.ts`](./packages/client/batcher/config.ts) | Hardhat account #0 |
| `WALLET_PRIVATE_KEY` | [`batcher/calls.ts`](./packages/client/batcher/calls.ts) | Hardhat account #0 |
| `MIDNIGHT_ADDRESS` | [`midnight/faucet.ts`](./packages/shared/contracts/midnight/faucet.ts), set by `/api/faucet` | — |
| `EFFECTSTREAM_STDOUT` | [`scripts/start.ts`](./packages/client/node/scripts/start.ts) | unset; when set, disables tmux/TUI/collector and logs to stdout |
| `RUN_IN_DOCKER` | set by the [`Dockerfile`](./Dockerfile) | — |

Everything else is hard-coded for the local stack, and all of it lives in
[`localhostConfig.ts`](./packages/shared/data-types/src/localhostConfig.ts): the EVM side is
...
`localhostConfig.ts`, [`batcher/config.ts`](./packages/client/batcher/config.ts) and
[`balanceOf.ts`](./packages/frontend/client/src/balanceOf.ts), set real

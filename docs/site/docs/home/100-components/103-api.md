...
The Paima Engine Node runs a powerful, built-in HTTP server that exposes a RESTful API. This API is the primary way for your [Frontend (dApp)](./115-frontend.md) and other external services to read the real-time, aggregated state of your application.
...
Your Paima Engine node comes with a set of default endpoints to query system status and generic data structures.
...
While the built-in endpoints are useful, you will often need to create custom routes that expose specific, aggregated data tailored to your application's needs. Paima Engine makes this easy by allowing you to define a custom `apiRouter` function.

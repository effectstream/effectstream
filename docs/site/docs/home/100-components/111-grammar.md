...
```ts
import { mapPrimitivesToGrammar } from "@paima/concise";
...
2.  **Prefix Matching**: The Statestream parses the JSON and inspects the first element (`"attack"`) to find the matching rule in your `grammar.ts` file.
...
Statestream reserves the `&` prefix for a suite of powerful, built-in system commands. You do not need to define these in your grammar file; the engine handles them automatically.

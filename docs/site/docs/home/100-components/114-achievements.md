...
Statestream includes a built-in implementation of the **Paima Request for Comments #1 (PRC-1)**, an open standard for exposing in-game achievements. By following this standard, your dApp can broadcast player accomplishments in a consistent, interoperable format.
...
Statestream takes care of the rest, automatically exposing this information through a standardized, PRC-1 compliant API.
...
// Export the metadata to enable the Statestream's built-in achievement API.
...
This `set` operation generates a database update that the Statestream will apply atomically with the rest of your STF's state changes.
...
Once enabled, your Statestream node will automatically serve the PRC-1 compliant API endpoints. Other developers, services, or even your own frontend can then query this data.

...
The `PaimaL2Contract` is a specialized, gas-efficient smart contract that serves as the primary "mailbox" or data entry point for your Statestream application. While Paima can monitor any contract, the `PaimaL2Contract` is optimized for submitting user actions and game moves directly to your state machine.

Its design is intentionally simple: its main job is to accept arbitrary data from a user, wrap it in an event, and securely log that event on the blockchain for the Statestream to process.
...
The `PaimaL2Contract` is the critical on-chain starting point that triggers your off-chain logic. The connection happens through a precise sequence of steps orchestrated by the Statestream:
...
3.  **Sync Service Detection**: The Statestream's **Sync Service**, which is constantly monitoring the blockchain, has a **Primitive** configured to listen specifically for the `PaimaGameInteraction` event from your contract's address.
...
sequenceDiagram
    ...
    participant Statestream (Sync Service)
    participant Statestream (Grammar Parser)
    participant Statestream (State Machine)
    ...
    Statestream (Sync Service)->>EVM Blockchain (PaimaL2Contract): [Primitive] Detects Event
    Statestream (Sync Service)->>Statestream (Grammar Parser): Passes raw data: ["attack","p1","m7"]
    Statestream (Grammar Parser)->>Statestream (State Machine): Parses input and identifies 'attack' prefix
    Statestream (State Machine)->>Statestream (State Machine): Executes 'attack' STF with parsed data
```
...
While most of your application's grammar is custom-defined to handle your specific game logic, Statestream reserves a special prefix, `&`, for a set of powerful, built-in system commands. These commands provide core functionalities common to most decentralized applications, including input batching and a flexible account management system.

These commands are processed directly by the Statestream before your custom State Transition Functions (STFs) are run.
...
A key feature of this system is its robustness & security features. Each input within the batch is a string. If one of the inputs is malformed or invalid according to your grammar, the Statestream will simply skip that single input and continue processing the rest of the batch. This prevents a single bad actor or a frontend bug from causing an entire batch of valid transactions to fail.

...
# Start the EffectStream Node (compiles contracts, builds and serves the frontend, and starts the full local stack)
bun run dev
```
...
**Important**: Always run `bun run --cwd packages/client/database pgtyped:update` after modifying SQL queries, as the type definitions need to be regenerated.
...
The frontend uses esbuild for bundling. When you run `bun run dev`, the frontend is automatically built and served. To manually rebuild:

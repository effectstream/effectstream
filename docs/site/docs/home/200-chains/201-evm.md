...
*   **`PrimitiveTypeEVMEffectstreamL2`**: Listens for the `PaimaGameInteraction` event on the `PaimaL2Contract`.
...
If you are using the `PaimaL2Contract` to submit generic game inputs, use the built-in `EffectstreamL2DefaultAdapter`.

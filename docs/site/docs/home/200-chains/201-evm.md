...
**Example:** A custom adapter that calls a `transferToMidnight` function on an ERC1155 contract (from the `multi-chain-token-transfer` template).
...
Use `launchEvm` from `@effectstream/orchestrator/launch-evm` to spin up a local Hardhat node and deploy contracts automatically.
...
The second argument tells the launcher where to resolve the package from — either `{ cwd }` (a directory) or `{ resolveFrom }` (a path to resolve the package name against). See [Process Orchestrator](../100-components/106-processes.md) for the full config shape.
...
```json
{
  ...
  "scripts": {
    ...
    "deploy:standalone": "bun run deploy:clean && bun run build && (((bun run chain:start) &) && (bun run chain:wait) && (bun run deploy) && (bun run chain:stop)) || true",
    "deploy": "bun run deploy:clean && bun run deploy.ts",
...
The remapping scripts default to the `@effectstream` scope, so no `--package` flag is needed. This is the exact script set `launchEvm` expects — it validates their presence and fails with a list of what is missing.

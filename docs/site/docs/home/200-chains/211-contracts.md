...
- **`PaimaLaunchpad`** + **`PaimaLaunchpadFactory`** - pre-order launchpad with reward tiers, package bundles, and cart checkout. Each campaign is its own deployment. Currently shipped inside [`templates/preorder/packages/contracts-evm/src/contracts/`](https://github.com/effectstream/effectstream/tree/main/templates/preorder/packages/contracts-evm/src/contracts) rather than `@effectstream/evm-contracts`. See the [NFT Launchpad blog post](/docs/blog/nft-launchpad) and the [`preorder` template](https://github.com/effectstream/effectstream/tree/main/templates/preorder).
...
- **`hololocker`** - locks an L1 Cardano NFT into a script address so it can be "projected" into an EffectStream L2 game state, then released back on a cooldown. Exposes both spend and mint validators. Source: [`hololocker.ak`](https://github.com/effectstream/effectstream/blob/main/e2e/shared/contracts/cardano/hololocker-demo/validators/hololocker.ak). Used by the [`projected-nft-preorder` template](https://github.com/effectstream/effectstream/tree/main/templates/projected-nft-preorder).

### Examples

- **`hello`** - minimal Aiken validator used by the e2e test suite. Reference for the shape of a custom validator inside an EffectStream template. Source: [`hello.ak`](https://github.com/effectstream/effectstream/blob/main/e2e/shared/contracts/cardano/hello/validators/hello.ak).

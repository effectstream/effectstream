...
const EFFECTSTREAM_ENV = process.env.EFFECTSTREAM_ENV;
if (EFFECTSTREAM_ENV) {
  config({
    path: `.env.${EFFECTSTREAM_ENV}`, // Uses .env_<EFFECTSTREAM_ENV>
  });
}
...
export class ENV {
  ...
  static set ORCHESTRATOR_PORT(port: number) {
    process.env[definitions.ORCHESTRATOR_PORT.key] = String(port);
  }

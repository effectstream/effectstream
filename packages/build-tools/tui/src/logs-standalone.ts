...
class LogsViewer {
  ...
  constructor() {
    ...
    const calledFrom = getEnv("INIT_CWD") ?? process.cwd();
    const envLogsPath = getEnv("LOGS_PATH");
...
// Handle graceful shutdown (Node-style; Deno 2 has node compat for process)
process.on("SIGINT", () => {
  console.log("\n👋 Stopping log viewer...");

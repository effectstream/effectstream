...
`deno task -f @[scope]/evm-contracts build`

# Deploy Contracts

`deno task -f @[scope]/evm-contracts deploy:standalone`
...
and run `deno task -f @[scope]/evm-contracts build`
...
`./ignition/modules/my-contract-module.ts`
...
Run `deno task -f @[scope]/evm-contracts deploy:standalone`

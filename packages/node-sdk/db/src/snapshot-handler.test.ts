...
async function makeSnapshotDir(): Promise<string> {
  return await Deno.makeTempDir({ prefix: "snapshot-retention-test-" });
}

/**
 * Seeds `dir` with one file per hour for `hours` hours, mtimes descending
 * from `now` (offset 0h, 1h, ..., hours-1h). Returns the seeded paths.
 */
...
Deno.test("retention: single pass over 8 days populates all tiers", async () => {
  const dir = await makeSnapshotDir();
  // Pick a `now` aligned to an hour boundary so bucket math is predictable.
  ...
  // Expected (boundary-inclusive math, `now` aligned):
...
Deno.test("retention: bucket keys are stable across successive passes (regression)", async () => {
  ...
  for (let tick = 0; tick < totalTicks; tick++) {
    ...
    const path = `${dir}/snapshot-${iso}.dump`;
    await Deno.writeTextFile(path, "x");
    await Deno.utime(path, new Date(now), new Date(now));
    await applyRetentionPolicy(dir, DEFAULT_RETENTION, now);
  }

  const survivors = await listSnapshots(dir);
  // With the old (buggy) bucketing this would collapse to ~25 (just the
  // hourly tier) because 6h/daily tier "keepers" kept getting reassigned
  // and pruned every tick. With the fix we expect ~37 (hourly + 6h + daily).
  assert(
    survivors.length >= 32,
...
Deno.test("retention: lastDayHourly=false keeps all files in hourly tier", async () => {
  const dir = await makeSnapshotDir();
  const now = Math.floor(Date.now() / MS_24H) * MS_24H;
...
Deno.test("retention: last3DaysSixHourly=false keeps all files in 24h–72h tier", async () => {
  const dir = await makeSnapshotDir();
  const now = Math.floor(Date.now() / MS_24H) * MS_24H;
...
Deno.test("retention: lastNDaysDaily=1 deletes everything past 24h", async () => {
  const dir = await makeSnapshotDir();
  const now = Math.floor(Date.now() / MS_24H) * MS_24H;
...
Deno.test("retention: empty directory is a no-op", async () => {
  const dir = await makeSnapshotDir();
...
Deno.test("retention: missing directory is a no-op (warns, does not throw)", async () => {
  const dir = await Deno.makeTempDir({ prefix: "snapshot-retention-missing-" });
  await Deno.remove(dir);
  // Should not throw.
  await applyRetentionPolicy(dir, DEFAULT_RETENTION, Date.now());
});

Deno.test("retention: leaves non-snapshot files untouched", async () => {
  const dir = await makeSnapshotDir();
  const now = Math.floor(Date.now() / MS_24H) * MS_24H;
  ...
  await Deno.writeTextFile(`${dir}/README.md`, "keep me");
  await Deno.writeTextFile(`${dir}/snapshot-bad-name.txt`, "keep me");
  await Deno.writeTextFile(`${dir}/other.dump`, "keep me");

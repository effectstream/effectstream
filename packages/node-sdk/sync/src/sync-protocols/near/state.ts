...
import type { NearFetcher } from "./fetcher.ts";
...
> {
  constructor(
    ...
    readonly config: Extract<
      ...
      { networkType: ConfigNetworkType.NEAR }
    >,
    fetcher: NearFetcher,
    public readonly client: NearClient,
    dbConn: PoolClient,
  ) {
    super(
  ...
  override toRootPage(data: Output): RootPage {
    // NEAR timestamps are already in ms (converted in fetcher)
    return applyDelay(data.timestamp, this.config.syncProtocol.delayMs);
  }

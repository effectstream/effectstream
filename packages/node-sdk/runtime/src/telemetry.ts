...
// 2. Allowing `type: text` imports: https://github.com/denoland/deno/issues/25354
...
export function* initTelemetry(): Operation<void> {
  const sdk = new NodeSDK({
    ...defaultOtelSetup("paima-node", DenoConfig.version),
    // TODO: set OTEL_EXPORTER_OTLP_PROTOCOL to json for @paima/collector support until it supports protobuf
    instrumentations: [getNodeAutoInstrumentations()],
  });

  const tracer = opentelemetry.trace.getTracer("paima", DenoConfig.version);

...
Basal execution (2026-08-10): **pass**. The exact `live-write-test` Compose gate ran under project `effectstream-midnight-v2-c18-20260810-24718` with diagnostic interpolation value `24718` freshly checked free and no published ports. The wrapper first proved that disabled authorization and absent funding confirmation both exit zero with `providerInitialized: false` and `deploymentInitialized: false`; a supplied readable but malformed secret failed nonzero before loading the contract path. The authorized local substitute used a unique `c18-local-<pid>-<time>` run ID and the public undeployed genesis seed documented in `$HOME/midnight-ref-ai`, mounted read-only through a Compose secret. It deployed sink then gateway, proved one root call, verified the Keccak digest, call tree, local event, exact finalized indexed event, and emitted a redacted report with only public block/digest/count data. The fixture runner scanned all captured child output for the exact seed plus private-state/witness/proof-material markers and found none. `Dockerfile.dockerignore` excludes the seed fixture, the context-audit stage asserted it was absent, the final image contained no seed fixture, owned application/test sources contained no literal seed, and image history contained no literal seed. The `live-write-tests` service exited zero; the standard proof-server lifecycle/request messages contained no proof payload or witness internals. This checkpoint validates the canary implementation against the local substitute only; it does not claim a new hosted transaction.
...
Basal execution (2026-08-10): **pass**. Image `effectstream/midnight-v2:c20` ran with no network, a read-only root filesystem, dropped capabilities, and no published ports. Its five isolated phases passed: the Node 22 provider boundary (including authentication of the unchanged 16-file Ledger-8 tree), the real Ledger-v9/runtime-v4 WASM smoke, 23 shared Midnight tests with 84 assertions (including byte-for-byte unchanged API-v3 behavior), three template state-machine tests with nine assertions, and two CI-registration tests with 14 assertions. The registration tests additionally parse both workflow files as YAML, classify `midnight-stagenet-v2` as Docker-managed, and enforce the deterministic/live trust split. The exact hermetic Compose gate then passed under project `effectstream-midnight-v2-c20-20260810-24723`: it deployed both local contracts, authenticated the sink-then-gateway call tree, matched the canonical Keccak-256 result across return/state/event data, stored the event once, rejected it on replay, and exposed zero published ports. Its first attempt correctly caught a missing explicit `undeployed` identity in the tests service; the environment was fixed and the complete gate rerun rather than accepting partial evidence. Exact-project cleanup removed both attempts' containers, volumes, and private networks.

The unchanged `evm-midnight-v2` image was rebuilt and rerun with the same six-CPU/10-GB profile on freshly preflighted loopback ports 24730-24737. Its first start was invalid infrastructure evidence because Docker Desktop's shared VM filesystem had reached 100%, causing the legacy node to fail closed with `StorageOutOfSpace`; only reproducible image tags created by this task were removed, 17 GB free was verified from a disposable container, and the whole baseline was rerun. The valid retry reproduced the recorded 24-pass/1-fail state: chain, wallet, contract, cross-chain, and API checks passed, while only `Frontend vite build exits successfully` failed with the existing JavaScript heap exhaustion. No new legacy failure was introduced. Both new workflows parsed successfully inside the no-network C20 image and passed `actionlint` 1.7.7 by digest in a no-network container. The ordinary deterministic workflow skips Docker-managed templates, the dedicated hermetic job uses a freshly preflighted range above 10000, scheduled/manual live reads are read-only, and the environment-gated write job is manual-only with its seed secret unavailable to untrusted pull requests. The exact legacy container was removed automatically and ports 24730-24737 were released.
...
Basal execution (2026-08-10): **pass**. Freshly preflighted diagnostic value 24791 was free and was not published. A clean BuildKit context governed by `Dockerfile.dockerignore` built `effectstream/midnight-v2:c21`; a separate `--no-cache-filter call-tree-build` invocation then forced the README's standalone target to compile the sink, gateway, and manifest in order, including the two expected fail-closed reverse/renamed-sibling checks. The docs image inherited that authenticated artifact dependency and added the exported template documentation and referenced sources. Its container ran read-only with `--network none`, dropped all capabilities, and passed four documentation tests with 42 assertions. Every repository-local README link resolved inside the exported template; the Docker build, hermetic, read-only live, authorized-write, port-preflight, and exact-project cleanup commands matched real targets/profiles; the source-to-compile-to-deploy sequence matched `build:call-tree`; all four stagenet URLs were exact; and the security assertions covered public disclosure, Keccak-vs-SHA3, repeated `Unpaused` semantics, event finality/retention/dedup, whole-slot prerelease refresh, manual Turnstile funding, NIGHT/DUST readiness, immutable hosted deployments, scoped cleanup, and redacted troubleshooting. The test rejected host-specific absolute paths and seed-valued environment examples. No host project runtime or host package cache was used.

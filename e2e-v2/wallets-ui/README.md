...
`run-tests.ts` is wired into the `wallets` suite in `e2e-v2/runner.ts`.
It:
...
flows stay **manual** against the running e2e-v2 infrastructure:
...
per-chain launchers under `e2e-v2/*/launcher.cli.ts`) and connect to it

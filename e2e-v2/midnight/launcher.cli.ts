...
export default {
  processes: [
    ...
    ...launchMidnight("@e2e-v2/midnight-contracts", import.meta.dirname!, {
    ...
    // ── Sync ──────────────────────────────────────────────────────────────────
    { name: "sync", args: ["run", "e2e-v2/midnight/node.ts"], waitToExit: false, type: "system-dependency" as const, env: { PGLITE: "true" }, dependsOn: [DbNames.PGLITE_WAIT, MidnightNames.CONTRACT_DEPLOY] },

    // ── Batcher ───────────────────────────────────────────────────────────────
    { name: "batcher", args: ["run", "e2e-v2/midnight/batcher/main.ts"], stopProcessAtPort: [3334], waitToExit: false, dependsOn: [MidnightNames.CONTRACT_DEPLOY] },
